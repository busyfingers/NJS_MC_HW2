...
handlers._orders.post = data => {
    ...
    return new Promise((resolve, reject) => {
        if (cartId) {
            // Lookup the specified cart
            _data.read("carts", cartId).then(_cartData => {
...
handlers._orders.get = data => {
    ...
    return new Promise((resolve, reject) => {
        if (id) {
            ...
            _data.read("orders", id).then(_orderData => {
                // Assign retrieved data to variable in function scope 
                orderData = _orderData;
                // Get the token from the headers
                let token = typeof (data.headers.token) == "string" ? data.headers.token : false;
                // Verify that the given token is valid for the email
                return handlers._tokens.verifyToken(token, orderData.email);
            }).catch(err => {
                if (err.statusCode === 500) {
                    reject(response(404, {"error": "Order not found"}));
                } else {
                    throw err;
                }
            }).then(_ => {
