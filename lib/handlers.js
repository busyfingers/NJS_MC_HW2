    ...
    return new Promise((resolve, reject) => {
        if (email) {
            ...
            handlers._tokens.verifyToken(token, email).then(_ => {
                ...
                // TODO: implement. For now, just resolve
                return Promise.resolve();
            }).then(_ => {
                // Delete the user
            ...
            reject(reponse(400, { "Error": "Missing required field" }));
        }
    });
}
...
// Required data: email, item(s)
// Optional data: none
handlers._carts.put = data => {
    // Verify email
    let email = typeof (data.payload.email) == "string" && data.payload.email.trim().length > 0 ? data.payload.email.trim() : false;
    let items = helpers.validateItemCounts(data.payload.items) ? data.payload.items : false;

    console.log("validation: ", email, items);
    ...
    return new Promise((resolve, reject) => {
        if (email && items) {
            ...
            handlers._tokens.verifyToken(token, email).then(_ => {
                ...
                if (err.statusCode === 500) {
                    throw (response(400, {"error": "Invalid user"}));
                } else {
                    throw err;
                }
            }).then(userData => {
                if (userData.cart) {
                    ...
                    return Promise.resolve(userData.cart);
                } else {
                    throw (response(500));
                }
            }).then(cartId => {
                // Fetch the cart data
                return _data.read("carts", cartId);
            // }).catch(err => {
            //     if (err.statusCode === 500) {
            //         // We could end up here either if the user does not have a cart property 
            //         // or if there was a problem fetching the cart from storage
            //         throw (response(500, {"error": "Could not retrieve user's cart"}));
            //     } else {
            //         throw err;
            //     }
            }).then(cartData => {
                console.log("Got some data: ", cartData)
                Object.keys(items).forEach(key => {
                    let newQuantity = items[key];
                    console.log("Processing: ", key, " - ", newQuantity);
                    if (newQuantity === 0 && cartData.items[key] !== undefined) {
                        console.log("Found this item in the cart so removing it")
                        // Remove item from cart if quantity is changed to 0
                        delete cartData.items[key];
                    } else if (newQuantity > 0) {
                        console.log("Item is not in cart, adding it")
                        // Add item(s) to cart if the quantity is > 0
                        cartData.items[key] = items[key];
                    }
...
handlers._carts.get = data => {
    ...
    return new Promise((resolve, reject) => {
        if (id) {
            // Lookup the specified cart
            _data.read("carts", id).then(_cartData => {
                cartData = _cartData;
                // Get the token from the headers
                let token = typeof (data.headers.token) == "string" ? data.headers.token : false;
                // Verify that the given token is valid for the email
                return handlers._tokens.verifyToken(token, cartData.email);
            }).catch(err => {
                if (err && err.statusCode === 500) {
                    reject(response(404, {"error": "Cart not found"}));
                } else {
                    // If verifyToken fails, err is undefined
                    throw (response(403, { "error": "Missing required token in header or token is invalid" }));
                }
            }).then(_ => {
                resolve(response(200, cartData.items));
            }).catch(err => {
                reject(err);
            });
        } else {
            reject(response(400, {"error": "Invalid cart id"}));
        }
    })
}
